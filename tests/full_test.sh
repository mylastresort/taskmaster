...
# Test attach on a stopped job (should fail)
R=$(send_cmd "attach nonexistent")
echo "$R" | grep -q "not recognized" && pass "ATTACH-2" || fail "ATTACH-2: $R"

# Test attach on a running job (should get ATTACH OK)
R=$(printf "attach attachtest\r" | nc -w 2 -U /tmp/test_taskmaster.sock 2>/dev/null | head -c 50)
echo "$R" | grep -q "ATTACH OK" && pass "ATTACH-3" || fail "ATTACH-3: $R"

# Test attach with detach sequence
